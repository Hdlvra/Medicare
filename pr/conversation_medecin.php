...
if (!$client) {
    ...
    echo "<p style='text-align:center;'><a href='medecin_chats.php'>← Retour</a></p>";
    exit();
}
...
<head>
  <meta charset="UTF-8">
  <title>Chat avec <?= htmlspecialchars($client['prenom'] . ' ' . $client['nom']) ?></title>
...
<body>
  <div class="chat-container">
    <h2>Discussion avec <?= htmlspecialchars($client['prenom'] . " " . $client['nom']) ?></h2>
    ...
    <div class="messages" id="chatBox">
      <?php while ($msg = $msgs->fetch_assoc()): ?>
        <div class="message <?= $msg['expediteur'] === 'client' ? 'client' : 'medecin' ?>">
          ...
          <span class="timestamp"><?= date("d/m/Y H:i", strtotime($msg['date_envoi'])) ?></span>
        </div>
      <?php endwhile; ?>
    </div>
    ...
    <div class="btns">
      <a href="medecin_chats.php">
        <button type="button">← Retour aux chats</button>
      </a>
      <a href="fiche_patient.php?id=<?= $client['id'] ?>">
    <button type="button">Voir fiche du patient</button>
</a>
